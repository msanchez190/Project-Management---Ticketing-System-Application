      ...
      <div class="col">
        <q-card class="requests-card q-ma-lg">
          <q-card-section class="text-white bg-secondary">
            <div class="text-h6 text-left">Hardware Subcategory Types</div>
          </q-card-section>
          <q-separator />
          <div class="q-pa-lg">
            <!-- Categries Table -->
  ...
  methods: {
    getsubcategories() {
      axios.get(`${apiURL}/categories`).then((res) => {
        ...
        this.hardwareCategoryTypeID = this.mainCategoryData.find(
          (o) => o.TICKET_CATEGORY_DESC.toUpperCase() === "HARDWARE" // get hardware category id
        );
        axios
          .get(
            `${apiURL}/subcattype/${this.hardwareCategoryTypeID.TICKET_CATEGORY_ID}` // get sub categories by type id
          )
          .then((res) => {
